...
export async function fixGradientTransparency(page: Page): Promise<void> {
  await page.evaluate(`
     (() => {
       ...
       const toRgba0 = (r, g, b) => \`rgba(\${r}, \${g}, \${b}, 0)\`;

       function rasterize(gradStr, w, h) {
       ...
       for (const el of els) {
         ...
         if (!bg.includes("gradient") || !TRANSPARENT_RE.test(bg))
           continue;
